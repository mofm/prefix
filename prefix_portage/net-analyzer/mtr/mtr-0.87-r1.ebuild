...
#FILECAPS=( cap_net_raw ${EPREFIX}/usr/sbin/mtr )
PATCHES=(
...
pkg_postinst() {
	if [[ ${CHOST} == *-darwin* ]] ; then
		ewarn "For MacOS users: (Please be careful!)"
		ewarn "sudo chown root:wheel ${EPREFIX}/usr/sbin/mtr"
		ewarn "sudo chmod u+s ${EPREFIX}/usr/sbin/mtr"
		ewarn " "
	fi
	elog "${EPREFIX}/usr/sbin/mtr must have setuid "root""
	elog "Please read about potential security issues in ${EPREFIX}/usr/share/doc/${P}/SECURITY"
}
